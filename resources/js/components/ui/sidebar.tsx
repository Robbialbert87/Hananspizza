...
function Sidebar({
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        <SheetContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="bg-sidebar text-sidebar-foreground w-(--sidebar-width) p-0 [&>button]:hidden rounded-r-2xl border-r-4 border-r-amber-500 shadow-[10px_0_40px_-8px_rgba(0,0,0,0.3)]"
          style={
            {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-300 ease-out md:flex shadow-lg",
          side === "left"
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-7 w-7 transition-transform duration-300 ease-out hover:scale-110 active:scale-95", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
      {...props}
    >
      <span className="transition-transform duration-300 ease-out data-[state=collapsed]:rotate-180" data-state={state}>
